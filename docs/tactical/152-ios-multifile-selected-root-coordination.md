...
Status: **Complete on 2026-08-13.** Exact-target coordination, controlled
multifile physical evidence, the repository Big Buck Bunny public-swarm run,
system playback, cleanup, and repository gates pass. Tactical `151` resumes
...
The existing bounded physical-iOS seed harness now also generates a three-file
2 MiB fixture with nested parents and both file boundaries inside 256 KiB
pieces. Its ready record carries exact per-file lengths and SHA-1 values plus
the concatenated logical SHA-1. A focused fixture test independently verifies
the metainfo geometry and every generated byte before the harness is used on
the phone. Single-file mode remains the default for prior lifecycle evidence.

### Physical root-health correction

The first signed hardware build exposed one same-boundary regression before
content intake: the engine probes a platform root with an intentionally empty
relative path, while the initial exact-target helper rejected every empty
path. The bridge now treats only empty observation as the selected root itself,
opens it through a no-follow directory descriptor, and reports its actual
directory kind. Empty payload opens and deletes remain invalid. A simulator
regression covers this health request, and the repaired physical build reports
the qualified root healthy without changing its opaque identity or bookmark
policy.

### Controlled physical multifile evidence

The signed development app downloaded the controlled three-file 2 MiB fixture
through the production magnet, peer, storage-pool, Swift coordination, and
publication paths on the attached iPhone. Both file boundaries fall inside
256 KiB pieces. All eight pieces verified, the torrent reached 100%,
Published, and Seeding, and the Files view exposed `first.bin`, nested
`second.bin`, and `third.bin` with their exact fixture lengths and Available
state.

The published tree survived application termination and relaunch. Force
recheck reread the complete tree and returned to eight of eight verified
pieces. The first file opened the system share sheet through the published-file
handoff. Managed removal with file deletion returned the catalog to zero and
Apple's picker independently reported the selected root empty. The controlled
seed stopped and its temporary metainfo, payload, request text, and captures
were removed.

### Public-swarm and system-playback evidence

The exact `big-buck-bunny` magnet in `tests/live/torrents.json`, with info hash
`dd8255ecdc7ca55fb0bbf81323d87062db1f6d1c`, downloaded from the changing
public swarm into the same qualified external root. The app observed active
payload at 13.7 MB/s during one sample, then reached 276,445,467 bytes,
1,055 of 1,055 verified pieces, 100%, Published, and Seeding without a platform
deadline or hash failure. The Files view exposed the 140-byte subtitle,
276.1 MB MP4, and 310 KB poster as three Available files.

Apple Files opened the published MP4 in its system video presentation. Two
captures taken at different times showed distinct decoded movie frames,
proving playback advanced rather than displaying a static thumbnail. No
embedded RSTorrent player or media-serving path was involved. Managed removal
then returned RSTorrent to zero torrents and Apple Files independently showed
the qualified root empty. The app, Files, device automation transaction, and
all run-owned temporary captures were closed or removed. No device identifier,
bookmark, selected path, signing value, or peer endpoint is retained here.

### Closure gates

The following final gates pass:

- `cargo fmt --all -- --check`;
- `cargo clippy --workspace -- -D warnings`;
- `cargo test --workspace`;
- the complete `RSTorrentTests` Swift bundle on the iOS simulator;
- the controlled multifile fixture test;
- two signed physical-device builds and installs, including the root-health
  correction; and
- `git diff --check` before closure.

The generated application boundary did not change, so web, Kotlin generation,
and Android packaging were inapplicable rather than silently omitted. Rust's
three-lease test records a high-water of three and terminal zero cached/owned
handles with exact one-time release delivery. Swift namespace tests record
exact-target sibling overlap, same-target serialization, no-follow containment,
empty root observation, and idempotent cleanup. Successful physical
publication/removal occurs only after the affected pooled handles drain; the
transaction then terminated both product processes and the exclusive runner.

Every stopping condition is met. The supported claim is qualified on-device
selected-root multifile download, verification, publication, restart/recheck,
published-file handoff, managed removal, and system playback after completion.
iCloud, positively identified providers, embedded or progressive playback,
indefinite background work, and public distribution remain outside scope.
