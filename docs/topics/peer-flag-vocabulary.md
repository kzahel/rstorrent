...
comparison and real inspection use provide evidence.
...
| --- | --- | --- | --- |
| `incoming` | `I` | Remote peer initiated this connection | Direction already represented; runtime incoming intake remains test-only |
...
| `upload_allowed` | `U` | Peer is interested and we are not choking it | Contract fields exist; current engine marks them unavailable |
| `upload_choked` | `u` | Peer is interested but we are choking it | Contract fields exist; current engine marks them unavailable |
| `extension_protocol` | `x` | Peer supports the BEP 10 extension protocol | Negotiated and represented |
| `metadata_extension` | `m` | Peer advertised the BEP 9 `ut_metadata` extension | Contract field exists; current projection is unavailable |
...
| `optimistic_unchoke` | `O` | Peer occupies an optimistic unchoke slot | Reserved; upload policy not implemented |
...
- The engine does not yet project encryption, hole-punch, seed, upload-only,
  optimistic-unchoke, connection parole, or connection-scoped endgame facts.
- Incoming and uTP are representable and tested vocabulary but not ordinary
  runtime capabilities.
- Upload interest/choke and metadata-extension fields exist but are currently
