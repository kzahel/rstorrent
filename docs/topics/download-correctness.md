...
| DL-C34 | Published content is rechecked, newly required pieces become durable, and the process restarts; a separate catalog row is malformed. | Piece checkpoints cannot relabel payload ownership, incomplete verification remains a restartable fact, the published torrent resumes from exact rechecked evidence, and the malformed torrent is quarantined without preventing profile open. | Failing in an observed schema-13 profile: `record_pieces` persisted staging storage with published ownership and the contradiction aborted application startup. Accepted Tactical [`105`](../tactical/105-fact-based-persistence-and-recheck-containment.md) owns the schema, runtime, crash, containment, and migrated-product evidence. |
