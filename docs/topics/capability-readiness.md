...
| Per-request timeout and slow-peer handling | Implemented | deterministic, runtime | Expiry reduces a peer to one probe request and is independent of unrelated messages; adaptive RTT-derived deadlines remain absent. | [`download-correctness`](download-correctness.md) |
