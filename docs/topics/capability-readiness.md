...
| Multi-file mapping and selective files | Implemented | deterministic, runtime, interop, web | Path-backed live Normal/Skip and metadata-only intake pass; high/low scheduling and dynamic SAF descriptor acquisition are absent. | [`client-persistence`](client-persistence.md), [`download-correctness`](download-correctness.md), [`android-saf-storage`](android-saf-storage.md) |
...
| Android SAF storage and publication | Implemented | runtime, AVD, physical | The proven path still uses an eager fixed descriptor manifest; dynamic acquisition, a shared actual-FD bound, live selection, general root management, removable-media policy, and migration remain absent. | [`android-saf-storage`](android-saf-storage.md), [`client-persistence`](client-persistence.md) |
