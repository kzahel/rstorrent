...
Explicit maintainer direction on 2026-08-30 activates Tactical
...
root IDs. No implementation or capability evidence is claimed yet; exact
pairing, SAF picker and two-root lifecycle, same-device ARC routing, Local
Network Access, Compose coexistence, detached transfer, reconnect, revoke,
shutdown, and physical ChromeOS gates remain open.
...
- Execute **Tactical `194`** and prove the JSTorrent-shaped ChromeOS companion
  journey with the inverse RSTorrent ownership: one Android foreground
  application/engine/profile owner, an explicitly paired extension React
  presentation, extension-triggered SAF selection, retained old-root authority
  after a new root becomes current, and no raw IO daemon or second library.
...
| Desktop and ChromeOS extension bootstrap | Implemented | deterministic, desktop, physical ChromeOS | Tactical `166` adds the distinct bounded `com.jstorrent.rstorrent.native` compatibility/launch host, exact production and beta-extension origins, registration repair, sidecar packaging, and the Manifest V3 JSTorrent Beta seed. Tactical `167` adds the exact local Crostini handoff; Tactical `168` makes version `0.3.0` platform-aware. Permissions remain only `nativeMessaging` and `storage`: desktop gets native bootstrap, ChromeOS gets the exact published JSTorrent Android listing plus ChromeOS Linux, and unknown platforms retain both. Deterministic package gates, installed macOS native `hello`/cold launch, and physical ChromeOS chooser/Play-link/Crostini handoff pass. Active Tactical `194` may add an optional exact ARC host permission and Android control, but no such implementation or evidence exists yet. Hosted Windows/Linux package evidence and every current torrent-control transport remain open. | [`client-surfaces`](client-surfaces.md), [`product-surfaces-and-migration`](product-surfaces-and-migration.md), [`beta-release-readiness`](beta-release-readiness.md) |
| ChromeOS Android extension presentation | Active, not implemented | design, pinned JSTorrent product survey | Tactical `194` selects the familiar launch/pair/detach/reconnect journey while reversing ownership: Android retains the sole Rust application service, profile, engine, networking, hashing, and SAF authority, and the extension packages the shared React UI over the semantic WebSocket. The same slice adds extension-triggered SAF selection and retained grants so one current root serves new downloads without redirecting older torrents. Exact optional ARC host permission, current Local Network Access, Android approval, Host/Origin/bearer admission, picker/root-registry crash recovery, two-root lifecycle, same-device-only routing, shared Compose state, bounds, revocation, joined shutdown, and physical Chromebook evidence are all still open. Raw IO, media, legacy-state import, production-extension publication, and general remote control are excluded. | [`194`](../tactical/194-chromeos-android-extension-control.md), [`product-surfaces-and-migration`](product-surfaces-and-migration.md), [`application-connection-architecture`](application-connection-architecture.md) |
...
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; magnet and `.torrent` intake, one-root SAF setup/repair, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Completion/failure notifications, metered/VPN enforcement, retained multiple roots, search/plugins, playback, tracker mutation, and extension presentation remain unavailable; Tactical `194` jointly selects retained-root support and the extension presentation as the first migration-critical parity slice. | [`client-surfaces`](client-surfaces.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
