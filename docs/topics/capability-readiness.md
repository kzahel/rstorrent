...
| Multi-file mapping and selective files | Implemented | deterministic, runtime, interop, web | Path-backed live Normal/Skip and metadata-only intake pass; high/low scheduling and dynamic SAF descriptor replacement are absent. | [`client-persistence`](client-persistence.md), [`download-correctness`](download-correctness.md) |
