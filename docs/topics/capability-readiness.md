...
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; in-application and external `magnet:`/`.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Tactical `197` proves exact manifest/runtime filtering, one bounded ephemeral queue and provider job, generic root/start confirmation, hostile-provider bounds, temporary-grant revocation, exact controlled transfer, resource highs, privacy, and cleanup on API 34 without changing the generated contract. Physical Tactical `194` proves two independent grants, future-add defaulting, durable torrent binding, independent grant loss/repair, Compose convergence with the extension, and the exact same-device transport. Tactical `198` now supplies bounded native completion/fatal-repair notifications, default-on preferences, low/default/high channels, exact taps, initial/restart/recheck non-replay, denied/blocked visible-only ownership, and target-35 timeout shutdown; its deterministic, dual-ABI, and API 34/35 AVD gates pass, while its final physical companion matrix is blocked by the unavailable Chromebook. Tactical `199` now supplies default-off live unmetered-network enforcement, fail-closed startup, joined network generations, truthful waiting presentation, and intent-preserving automatic recovery; deterministic, dual-ABI, and genuine API 28/35 AVD campaigns pass, while its physical-phone handoff awaits explicit authorization. JSTorrent-shaped background/idle/completion/seeding lifetime remains absent with ready Tactical `200`. VPN enforcement, search/plugins, playback, low-battery shutdown, and tracker mutation also remain unavailable. Production replacement of `com.jstorrent.app` has the stronger handoff and feature-disposition ledger in `android-jstorrent-replacement`. | [`client-surfaces`](client-surfaces.md), [`android-jstorrent-replacement`](android-jstorrent-replacement.md), [`200`](../tactical/200-android-product-background-lifecycle.md), [`199`](../tactical/199-android-live-unmetered-network-enforcement.md), [`198`](../tactical/198-android-completion-and-attention-notifications.md), [`197`](../tactical/197-android-external-torrent-intake.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
