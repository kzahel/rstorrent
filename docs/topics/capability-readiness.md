...
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; magnet and `.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Physical Tactical `194` proves two independent grants, future-add defaulting, durable torrent binding, independent grant loss/repair, Compose convergence with the extension, and the exact same-device transport. Completion/failure notifications, metered/VPN enforcement, search/plugins, playback, and tracker mutation remain unavailable. | [`client-surfaces`](client-surfaces.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
