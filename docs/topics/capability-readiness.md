...
Explicit user direction on 2026-09-01 selects ready Tactical
[`207`](../tactical/207-android-safe-reset-and-clear-data.md) for the complete
Android reset/clear-data disposition. It defines one atomic configured-default
engine-settings reset and one durable joined clear workflow with
unchecked-by-default Keep versus exact registered-payload deletion. The
workflow preserves unrelated root content, reports precise partial failure,
recovers across process death, releases SAF grants only after torrent work,
...
- Implement **Tactical `207`**: enable atomic engine-settings reset and add
  ...
  preserve unrelated root content, recover across process death, and release
  grants only after successful or explicitly downgraded removal.
