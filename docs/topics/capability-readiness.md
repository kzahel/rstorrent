...
- Execute **Tactical `196`** and turn the retained lower-rtc experiment into a
  default-compiled but lazy desktop/headless remote **Save file...** path.
  Authenticate bounded signaling in the existing encrypted circuit, preserve
  zero payload bytes through the relay, gather direct candidates through one
  documented public Cloudflare STUN endpoint, expose default-on operator
  control/audit, and fail truthfully without TURN or OPFS.
...
| Direct remote file streaming | Experimental, product integration ready | deterministic, runtime, real browser, current-host release | Tactical `195` selects lower `rtc` 0.20.4 and retains one lazy supervised endpoint plus bounded range codec. Chromium/Firefox verify concurrent exact ranges, cancellation, hostile-control survival, and 8/64-MiB incremental OPFS streams at 23.5--25.8 MiB/s with roughly 5.2 MiB active RSS increment and zero terminal endpoint owners. Post-completion Playwright WebKit reruns authenticate DTLS and complete exact range traffic; the reproducible failure is OPFS in Playwright's non-persistent context, while a persistent-context primitive probe passes. The macOS ARM64 cost is about 3.6 MiB stripped or 1.6--1.8 MiB compressed/package. Ready Tactical `196` owns default desktop/headless compilation, authenticated product signaling, completed-file Save, public STUN, operator control/audit, and direct-only failure. Active incomplete files, Open/Play, TURN, UPnP, deployment, support claims, and broader platform evidence remain absent. | [`direct-remote-file-streaming`](direct-remote-file-streaming.md), [`195`](../tactical/195-webrtc-direct-file-feasibility-spike.md), [`196`](../tactical/196-remote-direct-file-product-integration.md) |
