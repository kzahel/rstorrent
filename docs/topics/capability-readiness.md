...
| Direct remote file streaming | Experimental, default-off | deterministic, runtime, real browser, current-host release | Tactical `195` selects lower `rtc` 0.20.4 and retains one lazy supervised endpoint plus bounded range codec. Chromium/Firefox verify concurrent exact ranges, cancellation, hostile-control survival, and 8/64-MiB incremental OPFS streams at 23.5--25.8 MiB/s with roughly 5.2 MiB active RSS increment and zero terminal endpoint owners. The macOS ARM64 cost is about 3.6 MiB stripped or 1.6--1.8 MiB compressed/package. Playwright WebKit times out before ICE/DTLS; active incomplete files, native Download/Open/Play, production signaling, STUN/TURN/NAT breadth, cross-platform package size, UI, default enablement, and support claims remain absent. | [`direct-remote-file-streaming`](direct-remote-file-streaming.md), [`195`](../tactical/195-webrtc-direct-file-feasibility-spike.md) |
