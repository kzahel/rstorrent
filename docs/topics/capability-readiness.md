...
- Finish the remaining physical ChromeOS gate for **Tactical `198`**. The
  native implementation, API 34/35 AVD campaigns, exact AVD notification taps,
  permission/channel lifecycle, target-35 timeout, dual-ABI build, and
  repository gates pass. A later authorized ChromeOS 150/API-33 campaign also
  passes denied-visible-only behavior, Compose-explained permission grant,
  companion disconnect/reconnect, the real ongoing-notification **Stop**
  action, listener refusal, and exact package/credential/power cleanup. Exact
  physical completion and repair notification taps remain the only open cases.
