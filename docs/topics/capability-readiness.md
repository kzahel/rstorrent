...
- Finish the physical phone and ChromeOS qualification for **Tactical `200`**
  after explicit exact-target authorization. The pure policy, one service
  owner, default-off controls, companion count watch, fail-closed sticky
  recovery, Android-15 quota fence, connected API 28/35 suites, controlled
  Home/reopen, API-35 task removal, process recovery, completion, background
  upload, shortened timeout, dual-ABI build, and repository gates pass. No
  physical target was used; `JAR-009` stays open only for those two campaigns.
...
- Finish the physical ChromeOS gate for **Tactical `198`**. The native
  implementation, API 34/35 AVD campaigns, exact notification taps,
  permission/channel lifecycle, target-35 timeout, dual-ABI build, and
  repository gates pass. The authoritative Chromebook doctor currently finds
  SSH unreachable and the profile locked, so the companion denial,
  disconnect/reconnect, tap, and terminal cleanup matrix cannot run until the
  testbed receives physical VT2/login recovery.
...
| Android Compose foreground client | Implemented; background lifecycle AVD-qualified | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; in-application and external `magnet:`/`.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Tactical `197` proves exact manifest/runtime filtering and hostile-provider bounds. Physical Tactical `194` proves two-root SAF and the exact same-device extension transport. Tactical `198` supplies bounded completion/fatal-repair notifications and target-35 timeout shutdown; its remaining physical notification matrix stays open. Tactical `199` supplies default-off live unmetered enforcement and fail-closed network generations; its physical handoff awaits authorization. Tactical `200` now supplies explicit default-off background continuation, authoritative active-work admission, activity handoff, default completion shutdown, optional background seeding, count-only companion grace, intent-preserving joined shutdown, sticky recovery, and persistent Android-15 quota fencing. Connected API 28/35 plus genuine controlled Home/reopen, API-35 task removal, process recovery, completion, background upload, shortened timeout, exact cleanup, and both native ABIs pass. Its exact-target phone and ChromeOS closure remains authorization-gated, so `JAR-009` is not yet closed. VPN enforcement, search/plugins, playback, low-battery shutdown, and tracker mutation remain unavailable. Production replacement of `com.jstorrent.app` has the stronger handoff and feature-disposition ledger in `android-jstorrent-replacement`. | [`client-surfaces`](client-surfaces.md), [`android-jstorrent-replacement`](android-jstorrent-replacement.md), [`200`](../tactical/200-android-product-background-lifecycle.md), [`199`](../tactical/199-android-live-unmetered-network-enforcement.md), [`198`](../tactical/198-android-completion-and-attention-notifications.md), [`197`](../tactical/197-android-external-torrent-intake.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
