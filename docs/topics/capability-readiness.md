...
- Execute **Tactical `199`** for Android live unmetered-network enforcement.
  Add one default-off cost preference, ordered default-network observer, and
...
| Offline, loopback-only, and online egress policy | Implemented | deterministic, runtime, web, AVD | Policy is fixed for one service lifetime. Android unmetered-network enforcement remains absent with ready Tactical `199`; VPN and proxy controls remain absent and separate. | [`application-control`](application-control.md), [`199`](../tactical/199-android-live-unmetered-network-enforcement.md) |
