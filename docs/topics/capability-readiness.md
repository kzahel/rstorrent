...
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; in-application magnet and `.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Physical Tactical `194` proves two independent grants, future-add defaulting, durable torrent binding, independent grant loss/repair, Compose convergence with the extension, and the exact same-device transport. External Android intent intake remains absent with ready Tactical `197`; completion/failure notifications and JSTorrent-like permission transparency remain absent with ready Tactical `198`; unmetered-network enforcement remains absent with ready Tactical `199`. VPN enforcement, search/plugins, playback, and tracker mutation also remain unavailable. Production replacement of `com.jstorrent.app` has the stronger handoff and feature-disposition ledger in `android-jstorrent-replacement`. | [`client-surfaces`](client-surfaces.md), [`android-jstorrent-replacement`](android-jstorrent-replacement.md), [`199`](../tactical/199-android-live-unmetered-network-enforcement.md), [`198`](../tactical/198-android-completion-and-attention-notifications.md), [`197`](../tactical/197-android-external-torrent-intake.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
