...
- **Product uTP path-MTU discovery.** Reactivated Tactical
  ...
  `rustix`; both Android ABIs cross-build. macOS remains fixed at 548 and the
  tactical is at its required review before adding the recommended focused
  safe socket-option dependency. Accepted implementation then continues
  through dynamic MTU, controlled performance/interop, macOS, and Android
  evidence.
...
required macOS dependency review is current and product behavior remains
fixed at 548.
Tactical
