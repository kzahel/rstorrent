...
- Execute decision-complete **Tactical
  [`193`](../tactical/193-stateless-foreground-downloader.md)** for one native
  finite downloader over the existing ephemeral application service: all
  files by default, exact BEP 53 `so` selection when present, direct final
  paths, conservative existing-data checking, same-user output-root exclusion,
  transient selective boundary storage, and native macOS/Linux/Windows
  evidence. It adds no durable resume, daemon, mobile CLI, package, or release.
...
| Stateless foreground downloader | Active | planned deterministic, runtime, interop, native macOS/Linux/Windows | Tactical `193` composes one ephemeral application owner rather than a second engine path, downloads all files except an explicit magnet BEP 53 `so` selection directly to final paths, conservatively rechecks prior payload, exits without seeding, and uses a cooperative same-user output-root lock plus transient auxiliary part storage. Durable resume, multiple torrents, daemon/RPC behavior, mobile CLI surfaces, packaging, and cross-product storage leasing are excluded. | [`193`](../tactical/193-stateless-foreground-downloader.md), [`direct-filesystem-storage`](direct-filesystem-storage.md), [`client-persistence`](client-persistence.md) |
...
| Local owner-password relay validation | Implemented internal mode, not a supported capability | deterministic, persistence, runtime, web, desktop, real browser | Tacticals `190` and `192` implement RFC 9807 OPAQUE, measured Argon2id, one native/Wasm record core, portable host identity, non-extractable private-browser P-256 continuity, challenge-bound automatic resume, exact revocation and a complete bounded security ledger. Desktop and configured headless own the same joined runtime; a separate loopback TLS relay stores only routing authority and forwards opaque bounded records. Release-built Chrome passes private/shared choice, reload and process restart, phone viewport, automatic relay restart, local installed-layout rollback, remote audit, revocation and blocking changed-host recovery under strict CSP with zero service workers. Bulk/media remain rejected. No external listener, account, public client/relay, DNS/TLS operation, Internet-path evidence, stable wire or support claim exists. | [`remote-access-authentication`](remote-access-authentication.md), [`application-connection-architecture`](application-connection-architecture.md), [`192`](../tactical/192-production-owner-relay-access.md) |
