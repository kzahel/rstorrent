...
Android, and physical ChromeOS gates. The tactical is blocked because ChromeOS
forwards the Android wildcard listener through Wi-Fi; another LAN device
reached the real hello route with caller-controlled expected Host/Origin. A
non-LAN-reachable OS transport decision is required before release evidence can
resume.
...
- Resume **Tactical `194`** only after selecting a non-LAN-reachable
  ChromeOS/ARC transport. The one-owner paired React journey, retained-root
  authority, repair, coexistence, detached transfer, and cleanup pass, but the
  wildcard Android listener is physically reachable through Chromebook Wi-Fi.

### Ready

- Execute **Tactical `195`** and determine whether an optional lazy pure-Rust
  WebRTC DataChannel is a sound direct remote-file transport. Compare
  high-level and Sans-I/O candidates, prove real verified ranges and one
  bounded browser consumption path, and record exact feature-off/on binary,
  package, dependency, idle, active, and cleanup costs before recommending
  product integration or rejection.
...
| Direct remote file streaming | Planned experiment | design | Tactical `195` is ready to compare pure-Rust WebRTC endpoints, prove lazy browser DataChannel ownership and exact verified file ranges, investigate one bounded browser download/seek path, and measure feature-off/on release binary and package cost. No dependency, product UI, default enablement, direct-file wire, STUN/TURN service, public reachability, or support claim is selected yet. | [`direct-remote-file-streaming`](direct-remote-file-streaming.md), [`195`](../tactical/195-webrtc-direct-file-feasibility-spike.md) |
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; magnet and `.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Physical Tactical `194` proves two independent grants, future-add defaulting, durable torrent binding, independent grant loss/repair, and Compose convergence with the extension. Completion/failure notifications, metered/VPN enforcement, search/plugins, playback, tracker mutation, and a releaseable extension transport remain unavailable. | [`client-surfaces`](client-surfaces.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
