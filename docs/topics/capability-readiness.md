...
- **Product uTP path-MTU discovery.** Reactivated Tactical
  ...
  `dontfrag 1.0.1` also passes actual macOS set/get/exact-restore proof. Stage
  3 now adds deterministic revalidation and downward recovery before dynamic
  product MTU, controlled performance/interop, and Android runtime evidence.
...
approved target-specific adapter passes actual macOS option proof, Stage 3 is
current, and product behavior remains fixed at 548.
Tactical
