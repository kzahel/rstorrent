...
- Execute **Tactical `197`** for bounded Android external `magnet:` and
  `content://` `.torrent` intake. Keep `MainActivity` as the sole exported
  activity, retain one service-owned ephemeral queue and joined provider job,
  and reuse the existing root/start/add application semantics without a new
  Rust or generated command.
- Finish qualification evidence for **Tactical `196`**. The implementation
  ...
  payload. The current-host production verifier and native Linux ARM64 build
  pass. Native Windows with a complete C toolchain, an independent-network
  selected pair, and a real streaming save picker remain open.
...
| Direct remote file streaming | Implemented, unadvertised; qualification active | deterministic, runtime, real browser, current-host release, native Linux ARM64 build | Tactical `195` selects lower `rtc` 0.20.4 and records the browser, resource, and package feasibility evidence. Tactical `196` now default-compiles the lazy endpoint in desktop/headless products, authenticates bounded RDF signaling inside the encrypted circuit, and exposes completed-file **Save file...**, public Cloudflare STUN, strict UUIDv4 `.local` mDNS, operator control, live resources, stop, and redacted audit. The production-shaped verifier saves an exact 1-MiB completed file through real TLS and the opaque relay while retaining zero relay payload and zero terminal direct owners; its login, resume, restart, rollback, revocation, audit, and hostile-circuit matrix passes. Chromium, Firefox, Playwright WebKit, and native probes gather public-STUN candidates; native Linux ARM64 compiles the default graph. Native Windows with a complete C toolchain, an independent-network selected pair, and a real streaming save picker remain open, so the capability is not advertised or supported. Active incomplete files, Open/Play, TURN, UPnP, deployment, and mobile hosting remain absent. | [`direct-remote-file-streaming`](direct-remote-file-streaming.md), [`195`](../tactical/195-webrtc-direct-file-feasibility-spike.md), [`196`](../tactical/196-remote-direct-file-product-integration.md) |
| Android Compose foreground client | Implemented | deterministic, runtime, Android build, AVD, physical | The maintained Material 3 product provides the JSTorrent-shaped Library, six-tab torrent detail, Speed, dual-family DHT, structured Logs, and Settings hierarchy with RSTorrent branding. One service-scoped owner consumes every Android-relevant bounded projection; in-application magnet and `.torrent` intake, retained multi-root SAF setup/current/repair/removal, High/Normal/Skip file priority/open, torrent and queue actions, backed settings including session/per-torrent transfer limits and active-work sleep inhibition, activity/process recovery, and controlled concurrent downloads pass. Physical Tactical `194` proves two independent grants, future-add defaulting, durable torrent binding, independent grant loss/repair, Compose convergence with the extension, and the exact same-device transport. External Android intent intake remains absent with active Tactical `197`; completion/failure notifications and JSTorrent-like permission transparency remain absent with ready Tactical `198`; unmetered-network enforcement remains absent with ready Tactical `199`. VPN enforcement, search/plugins, playback, and tracker mutation also remain unavailable. Production replacement of `com.jstorrent.app` has the stronger handoff and feature-disposition ledger in `android-jstorrent-replacement`. | [`client-surfaces`](client-surfaces.md), [`android-jstorrent-replacement`](android-jstorrent-replacement.md), [`199`](../tactical/199-android-live-unmetered-network-enforcement.md), [`198`](../tactical/198-android-completion-and-attention-notifications.md), [`197`](../tactical/197-android-external-torrent-intake.md), [`194`](../tactical/194-chromeos-android-extension-control.md) |
