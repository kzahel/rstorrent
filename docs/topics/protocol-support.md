...
remote-mapped direct-public-path transfer with RSTorrent as leecher. The BEP 29
claim remains **Unsupported** because ordinary product dialing/listening and
UDP reachability, a repeatable WAN cohort, IPv6 uTP, MSE composition,
transport policy, and product presentation are not implemented. Closed
...
readiness but deliberately does not change the shipped default or claim.
...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Unsupported | Completed Tacticals [`119`](../tactical/119-deterministic-utp-transport-core.md) and [`121`](../tactical/121-deterministic-utp-loss-congestion-and-mtu.md) add the dependency-free v1 codec and bounded deterministic connection/reliability/receive state, exact receive credit and packetization, delayed ACKs, retransmission execution, fixed-point RFC 6817 congestion and pacing, and binary path-MTU discovery. Fixed exact-hash simulations pass impairment, pressure, black-hole, and foreground thresholds. Tactical [`125`](../tactical/125-shared-udp-utp-runtime-and-loopback-interop.md) adds bounded shared DHT/uTP routing, a supervised generation-fenced IPv4 runtime, ordered peer streams, controlled plaintext peer-wire/incoming-upload composition, and exact pinned-libtorrent loopback transfer in both roles. Tacticals [`127`](../tactical/127-mapped-utp-wan-interoperability.md) and [`130`](../tactical/130-utp-transport-solidification.md) prove both first-sample mapped-WAN directions, a deterministic six-profile real-socket matrix, hostile lifecycle bounds, and controlled diagnostic-MTU convergence. Completed Tactical [`131`](../tactical/131-bounded-product-utp-composition.md) composes fixed uTP into the application behind default-off `TcpOnly`/`PreferUtp` policy. Incoming and eligible IPv4/plaintext outgoing paths, exact transport projection, cancellation, and sequential TCP fallback retain one logical attempt and permit; the exact 2,097,883-byte application gate passes in incoming uTP, outgoing uTP, and TCP-only fallback roles. Completed Tactical [`132`](../tactical/132-utp-default-readiness-evidence.md) adds endpoint-scoped unknown/advertised/confirmed/suppressed memory in the existing bounded record, five-minute-to-one-hour re-probe, PEX refresh, exact joined outcome ownership, suppressed direct TCP, real-socket expiry recovery, and a 2026-08-11 ordinary-swarm observation that verified Big Buck Bunny metadata in 2.862383 seconds while observing both TCP and uTP, fixed 548-byte MTU, bounded queues, and terminal zero ownership. | Every shipped/default client remains TCP-only; no persisted setting, mapping, tracker/DHT incoming-endpoint advertisement, or presentation exists. The first attempt to a TCP-only endpoint still pays one uTP timeout, while repeats use direct TCP until bounded recovery. The fresh WAN cohort remains evidence-limited. Portable per-datagram fragmentation protection, Internet PMTU evidence, IPv6 uTP, MSE-over-uTP, incoming public reachability, default policy, and claim graduation remain absent. The claim can become **Partial** only through the separate product-default human decision and its bounded implementation evidence. |
