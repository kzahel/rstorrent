...
default/reachability scope and graduation evidence.
...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Unsupported | Completed Tacticals [`119`](../tactical/119-deterministic-utp-transport-core.md) and [`121`](../tactical/121-deterministic-utp-loss-congestion-and-mtu.md) add the dependency-free v1 codec and bounded deterministic connection/reliability/receive state, exact receive credit and packetization, delayed ACKs, retransmission execution, fixed-point RFC 6817 congestion and pacing, and binary path-MTU discovery. Fixed exact-hash simulations pass clean, jitter/reorder/duplicate, 1% loss, queue, clock wrap/drift, zero-window pressure, DF black-hole, and established TCP-like foreground thresholds. Tactical [`125`](../tactical/125-shared-udp-utp-runtime-and-loopback-interop.md) adds bounded DHT/uTP routing on the shared session UDP receiver, a supervised generation-fenced IPv4 runtime, ordered peer streams, controlled plaintext peer-wire/incoming-upload composition, and exact pinned-libtorrent loopback transfer in both roles. Tactical [`127`](../tactical/127-mapped-utp-wan-interoperability.md) proves a remote-mapped direct-public-path RSTorrent leecher. Closed Tactical [`130`](../tactical/130-utp-transport-solidification.md) proves the complementary locally mapped RSTorrent-seed direction; a deterministic six-profile real-socket matrix; hostile lifecycle bounds; and controlled diagnostic-MTU convergence. Completed Tactical [`131`](../tactical/131-bounded-product-utp-composition.md) composes the fixed runtime into the application behind default-off `TcpOnly`/`PreferUtp` construction policy. Incoming and eligible IPv4 plaintext outgoing application paths, exact transport projection, cancellation, joined timeout, and sequential TCP fallback retain one logical attempt and permit. The exact 2,097,883-byte application gate passes with one incoming uTP peer, one outgoing uTP peer, and one final TCP peer after three unanswered uTP datagrams; default TCP/Fast/MSE regressions and the workspace pass. | Every shipped/default client remains TCP-only; no persisted setting, mapping, advertisement, or presentation exists. A first TCP-only endpoint pays an uncached five-second uTP timeout. The fresh WAN cohort remains evidence-limited. Portable per-datagram fragmentation protection, Internet PMTU evidence, endpoint capability memory, IPv6 uTP, MSE-over-uTP, ordinary swarm evidence, default/reachability policy, and claim graduation remain absent. |
