...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Partial | Tacticals [`119`](../tactical/119-deterministic-utp-transport-core.md) and [`121`](../tactical/121-deterministic-utp-loss-congestion-and-mtu.md) add the dependency-free v1 codec and bounded deterministic reliability, receive-credit, packetization, delayed-ACK, retransmission, fixed-point RFC 6817 congestion/pacing, and path-MTU state. Tactical [`125`](../tactical/125-shared-udp-utp-runtime-and-loopback-interop.md) adds bounded shared DHT/uTP routing, supervised generation-fenced IPv4 runtime/streams, ordinary peer-wire/incoming-upload composition, and exact pinned-libtorrent loopback transfer in both roles. Tacticals [`127`](../tactical/127-mapped-utp-wan-interoperability.md) and [`130`](../tactical/130-utp-transport-solidification.md) prove both first-sample mapped-WAN directions, a six-profile real-socket matrix, hostile lifecycle bounds, and controlled diagnostic-MTU convergence. Tacticals [`131`](../tactical/131-bounded-product-utp-composition.md) and [`132`](../tactical/132-utp-default-readiness-evidence.md) compose fixed uTP into the application with incoming admission, eligible IPv4/plaintext selection, actual transport views, one-permit sequential TCP fallback, endpoint capability memory, suppression/recovery, PEX refresh, and one successful ordinary-swarm metadata observation. Completed Tactical [`133`](../tactical/133-utp-product-default-enablement.md) makes `PreferUtp` the common application construction default. Its exact 2,097,883-byte pinned-libtorrent cohort passes inherited-default incoming uTP, outgoing uTP, and TCP fallback with fixed 548-byte uTP, exact hashes, and joined cleanup; explicit `TcpOnly` retains ordinary TCP, Fast, and forced-MSE isolation. | The construction policy is not a persisted/user-visible setting. UDP mapping, tracker/DHT incoming-endpoint advertisement, public incoming reachability, IPv6 uTP, MSE-over-uTP, TCP/uTP racing, proxy semantics, portable per-datagram fragmentation protection, dynamic product MTU, and repeatable WAN-cohort evidence remain absent. A first TCP-only endpoint still pays one bounded uTP timeout; volatile suppression makes repeats use direct TCP until recovery. This is not a **Supported** claim. |
