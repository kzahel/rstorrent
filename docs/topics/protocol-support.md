...
| --- | --- | --- | --- |
| [BEP 3: The BitTorrent Protocol Specification](https://www.bittorrent.org/beps/bep_0003.html) | Partial | Strict bounded bencoding; distinct `length` and `files` publication shapes over one v1 storage/resume pipeline; raw-info SHA-1 identity; TCP handshake; keepalive, choke, unchoke, interested, not-interested, have, bitfield, request, cancel, and piece messages; bounded multi-peer block scheduling, request expiry and failover, strict endgame duplicates, whole-piece hash-failure recovery, all-wanted managed recheck, SHA-1 verification, and single-/multi-file publication. Deterministic, scripted adverse, controlled libtorrent, crash, and live publication evidence cover the implemented subset. | General payload upload, incoming peer service, a choking algorithm, HTTP trackers, and comparable ordinary-swarm completion performance remain absent. |
