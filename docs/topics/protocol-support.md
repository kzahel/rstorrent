...
plus pure-v2 magnet subsets sufficient for controlled verified downloads,
SHA-1/SHA-256 BEP 9 metadata exchange, authenticated BEP 52 hash exchange,
scheduled BEP 15 UDP tracker announces, an IPv4 Mainline DHT foundation, and
bounded
...
and pure-v2 magnet subsets; hybrid behavior, creation, and broader Merkle
exchange remain absent.
...
| [BEP 9: Extension for Peers to Send Metadata Files](https://www.bittorrent.org/beps/bep_0009.html) | Supported | Bounded v1 `btih` and pure-v2 `btmh` magnets, extension negotiation, metadata size and block bounds, at most two acquisition requests per download peer, request/data/reject messages, duplicate and ordering validation, cross-peer block assembly, identity-selected exact SHA-1 or SHA-256 verification, strict discovered-format admission, generation recovery, up to eight simultaneous dial/metadata work items, an independent progress deadline, inspectable acquisition state, and bounded diagnostic plus shared multi-peer application-listener metadata upload. Controlled libtorrent runs pass in both directions; the maximum receive proof transfers an exact 31,457,280-byte info dictionary in 1,920 blocks and requests, local upload serves every block of valid metadata up to 64 MiB, and Tactical [`155`](../tactical/155-v2-magnet-authenticated-hash-exchange.md) proves info-only pure-v2 intake and service before BEP 52 payload. | The first complete hash-verified download generation wins. Mixed `btih`/`btmh` identity and metadata-discovered hybrid adoption remain outside the claim; public-swarm discovery and completion remain variable rather than a reliability claim. |
...
| [BEP 52: The BitTorrent Protocol Specification v2](https://www.bittorrent.org/beps/bep_0052.html) | Partial | Tacticals [`143`](../tactical/143-dual-identity-and-persistence-foundation.md) and [`146`](../tactical/146-runtime-free-bep52-metainfo-geometry-merkle.md) add explicit identities, stable ownership, exact-byte v2 models, aligned geometry, strict complete piece layers, and bounded SHA-256 Merkle primitives. Completed Tactical [`151`](../tactical/151-complete-source-pure-v2-runtime-vertical.md) carries strict complete local pure-v2 `.torrent` bytes through selective path/SAF storage, Merkle verification, durable have, restart/recheck, publication, verified reads, active upload, completed seeding, and standard peer payload messages. Completed Tactical [`155`](../tactical/155-v2-magnet-authenticated-hash-exchange.md) adds exact `btmh` intake/export, SHA-256 info-only metadata, a bounded volatile authenticated hash catalog, hash request/hashes/hash-reject messages 21--23, hash-first selective payload, incomplete candidate refetch, complete-file local reconstruction, leaf-level corruption repair, and active/completed hash service. The full 32-byte identity remains authoritative; tracker, DHT, plaintext handshake, MSE, TCP, and uTP routing use the tagged 20-byte truncation. Exact pinned-libtorrent magnet transfer and hash exchange pass in both roles with peer-hint-only TCP, tracker/DHT/default-uTP, accepted uTP-only, forced RC4 MSE, selection promotion, restart, production browser, Tauri, Android AVD, iOS archive, and bounded-resource evidence. | Mixed identity, hybrid negotiation, live-owner reconciliation, dual-swarm verification, arbitrary Merkle base layers, durable sparse hash state, torrent creation, and a public-swarm reliability claim remain unsupported. The exact pure-v2 magnet subset accepts only hexadecimal `btmh:1220` identity and reconstructs or refetches hash authority conservatively after restart. |
...
6. retain the completed incoming service and uTP foundations plus the exact
   complete-source and magnet pure-v2 **Partial** claim; plan hybrid dual-swarm
   closure from [`bittorrent-v2-and-hybrid`](bittorrent-v2-and-hybrid.md)
   before expanding that claim, while hole punching and web seeds remain
   separate work.
