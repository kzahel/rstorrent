...
fallback and is now active. Its shared egress, exact socket-policy
restoration/replacement, revalidation/downward recovery, capability-gated
product default, controlled 1,500/1,280 paths, capped application/libtorrent
cohort, and five-pair efficiency gate pass. Android runtime and repository
closure remain; the **Partial** claim and every unrelated limit remain
unchanged.
...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Partial | Tacticals [`119`](../tactical/119-deterministic-utp-transport-core.md) and [`121`](../tactical/121-deterministic-utp-loss-congestion-and-mtu.md) add the dependency-free v1 codec and bounded deterministic reliability, receive-credit, packetization, delayed-ACK, retransmission, fixed-point RFC 6817 congestion/pacing, and path-MTU state. Tactical [`125`](../tactical/125-shared-udp-utp-runtime-and-loopback-interop.md) adds bounded shared DHT/uTP routing, supervised generation-fenced IPv4 runtime/streams, ordinary peer-wire/incoming-upload composition, and exact pinned-libtorrent loopback transfer in both roles. Tacticals [`127`](../tactical/127-mapped-utp-wan-interoperability.md) and [`130`](../tactical/130-utp-transport-solidification.md) prove both first-sample mapped-WAN directions, a six-profile real-socket matrix, hostile lifecycle bounds, and controlled diagnostic-MTU convergence. Tacticals [`131`](../tactical/131-bounded-product-utp-composition.md), [`132`](../tactical/132-utp-default-readiness-evidence.md), and [`133`](../tactical/133-utp-product-default-enablement.md) compose fixed uTP into the application and make `PreferUtp` the common construction default with incoming admission, eligible IPv4/plaintext selection, actual transport views, one-permit sequential TCP fallback, endpoint capability memory, suppression/recovery, PEX refresh, and one successful ordinary-swarm metadata observation. Active Tactical [`137`](../tactical/137-product-utp-path-mtu-discovery.md) adds safe fragmentation-protected sends on Linux/Android/macOS, exact shared-socket restoration and contaminated-generation repair, 15-minute revalidation/downward recovery, dynamic 548--1,472 product packetization only for verified IPv4 capability, and fixed-548 fallback otherwise. Controlled 1,500/1,280 paths select 1,457/1,269 bytes; five alternating pairs reduce median DATA packets 62.97%; and the exact rate-limited pinned-libtorrent application cohort passes in both roles. | The construction policy is not a persisted/user-visible setting. UDP mapping, tracker/DHT incoming-endpoint advertisement, public incoming reachability, IPv6 uTP, MSE-over-uTP, TCP/uTP racing, proxy semantics, destination-MTU persistence, and repeatable WAN-cohort evidence remain absent. Android runtime closure for Tactical `137` remains pending. A first TCP-only endpoint still pays one bounded uTP timeout; volatile suppression makes repeats use direct TCP until recovery. This is not a **Supported** claim. |
