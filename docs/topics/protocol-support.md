...
family TCP endpoint for an eligible completed seed. The bounded
`WANIPv6FirewallControl:1` subset is implemented and passes deterministic and
scripted-gateway evidence, but its physical off-LAN proof remains pending. It
...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Unsupported | None. | Peer transport is TCP only. The [`utp-transport-campaign`](utp-transport-campaign.md) topic records the adaptive source, ownership, implementation, and evidence strategy; bounded child tacticals have not started. |
