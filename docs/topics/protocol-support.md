...
| --- | --- | --- | --- |
| [BEP 3: The BitTorrent Protocol Specification](https://www.bittorrent.org/beps/bep_0003.html) | Partial | Strict bounded bencoding; v1 single- and multi-file info dictionaries; raw-info SHA-1 identity; TCP handshake; keepalive, choke, unchoke, interested, not-interested, have, bitfield, request, and piece messages; bounded multi-peer block scheduling, request expiry, failover, full-piece SHA-1 verification, and single-/multi-file publication. Deterministic, scripted adverse, and controlled libtorrent downloads cover the implemented subset. | No cancel message, general payload upload, incoming peer service, endgame duplicates, hash-failure recovery, choking algorithm, HTTP tracker, or broad ordinary-swarm completion claim. |
...
| [BEP 9: Extension for Peers to Send Metadata Files](https://www.bittorrent.org/beps/bep_0009.html) | Supported | Bounded v1 `btih` magnets, extension negotiation, metadata size and block bounds, at most two acquisition requests per peer, request/data/reject messages, duplicate and ordering validation, assembled info-hash verification, up to three simultaneous dial/metadata work items, an independent progress deadline, inspectable acquisition state, and a bounded diagnostic metadata uploader. Controlled libtorrent runs pass in both directions; repeated public DHT metadata acquisitions also pass. | The first complete hash-verified source wins; metadata blocks are not assembled across peers. V2 identities, BEP 53 selection, and general seeding are outside the claim. |
...
| [BEP 15: UDP Tracker Protocol for BitTorrent](https://www.bittorrent.org/beps/bep_0015.html) | Partial | Bounded connect and announce codecs, source/action/transaction/stride validation, IPv4 and IPv6 compact response parsing, DNS and destination policy, connect/announce retransmission, 30-second aggregate operation bounds, 60-second connection-token cache, multi-tracker fallback, failure backoff, success promotion, interval reannounce, and an explicit provisional port 6881. Scripted loss tests, controlled libtorrent tracker-only downloads, and a public metadata acquisition pass. | Runtime announces still use placeholder transfer counters, and port 6881 is not backed by an incoming listener; completed and stopped events, scrape, authentication, proxies, shared session budgets, and multiple simultaneous operations are absent. |
...
1. add core cancel/endgame behavior and hash-failure recovery under the
   existing bounded multi-peer BEP 3 subset;
2. finish the paired headless comparative smoke needed to measure live
   completion without adding a product UI;
3. measure and tune connection/picker policy against controlled and public
   evidence;
