...
| [BEP 29: uTorrent Transport Protocol](https://www.bittorrent.org/beps/bep_0029.html) | Unsupported | Completed Tactical [`119`](../tactical/119-deterministic-utp-transport-core.md) adds a runtime-free, dependency-free v1 packet codec and bounded deterministic connection, receive, sent-packet, ACK/SACK, loss-signal, RTT/RTO, FIN, and RESET state under 41 pure tests. It owns no socket and has exchanged no RSTorrent datagram. | Peer transport remains TCP only. LEDBAT/congestion, receive-window and packetization policy, MTU behavior, impairment evidence, shared-UDP/runtime ownership, ordered-stream and peer-wire integration, controlled interoperability, WAN/product policy, and support graduation remain later campaign gates. |
