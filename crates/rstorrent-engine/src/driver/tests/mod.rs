...
async fn serve_http_tracker_lifecycle(
    listener: TcpListener,
    info_hash: [u8; 20],
    peer: SocketAddr,
) -> Vec<String> {
    ...
    for _ in 0..3 {
        ...
        request_targets.push(request_target);
    }
    request_targets
}

async fn serve_declared_failure_http_tracker(listener: TcpListener) {
    let (mut stream, _) = listener
        .accept()
        .await
        .expect("accept failing HTTP tracker");
    read_test_http_headers(&mut stream).await;
    let body = b"d14:failure reason26:controlled tracker failuree";
    let response = format!(
        "HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
        body.len()
    );
    stream
        ...
        .await
        .expect("write failing HTTP tracker headers");
    stream
        .write_all(body)
        .await
        .expect("write failing HTTP tracker body");
    stream.shutdown().await.expect("close failing HTTP tracker");
}

async fn read_test_http_headers(stream: &mut TcpStream) -> Vec<u8> {
    let mut request = Vec::new();
    loop {
        let mut chunk = [0_u8; 1024];
        let length = stream
