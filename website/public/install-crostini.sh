...
PINNED_TAG="crostini-v0.1.0"
REPOSITORY="kzahel/rstorrent"
MANIFEST_NAME="rstorrent-crostini-release.manifest"
SIGNATURE_NAME="rstorrent-crostini-release.manifest.minisig"
...
# Verify the Minisign data signature. The optional trusted comment is not
# release data; every trusted value lives inside the signed manifest.
verify_minisign() {
    local data_file="$1"
    ...
    printf '\060\052\060\005\006\003\053\145\160\003\041\000' > "$public_der" || return 1
    dd if="$public_binary" bs=1 skip=10 count=32 status=none >> "$public_der" || return 1
    dd if="$signature_binary" bs=1 skip=10 count=64 status=none > "$signature_raw" || return 1
    case "$signature_algorithm" in
        4544) openssl dgst -blake2b512 -binary "$data_file" > "$signed_message" || return 1 ;;
        4564) cp "$data_file" "$signed_message" || return 1 ;;
        *)
...
signature_asset'
    ...
    if [ "$(sed -n '1p' "$manifest")" != "rstorrent-crostini-release-v1" ] ||
    ...
    local source_commit launch_protocol extension_id runtime manifest_asset signature_asset
    source_commit=$(manifest_value source_commit "$manifest") || return 1
    launch_protocol=$(manifest_value launch_protocol "$manifest") || return 1
    extension_id=$(manifest_value extension_id "$manifest") || return 1
    ...
    if [ "$RELEASE_ASSET" != "rstorrent-crostini-${RELEASE_VERSION}-${arch}.tar.gz" ] ||
...
download_release_metadata() {
    ...
    local temp_dir="$2"
    local tag="$PINNED_TAG"
    if [ -n "$requested_version" ]; then
        tag="crostini-v${requested_version}"
    fi
    ...
    curl -fSL --proto '=https' --proto-redir '=https' --max-time 30 \
        ...
        "${base_url}/${MANIFEST_NAME}" -o "$temp_dir/$MANIFEST_NAME" || return 1
    curl -fSL --proto '=https' --proto-redir '=https' --max-time 30 \
        --max-filesize "$MAX_METADATA_BYTES" \
...
validate_archive_listing() {
    ...
    if ! tar -tzf "$archive" > "$list_file"; then
        error "Release package is not a readable gzip-compressed tar archive."
        return 1
    fi
    ...
    while IFS= read -r entry; do
        ...
        if [ "$count" -gt "$MAX_ARCHIVE_ENTRIES" ]; then
            error "Release package contains too many entries."
            return 1
        fi
        case "$entry" in
            ''|/*|*\\*)
                error "Release package contains an unsafe path."
                return 1
                ;;
        esac
        normalized="${entry#./}"
        ...
        case "/${normalized}/" in
            */../*|*/./*)
                error "Release package contains path traversal."
                return 1
                ;;
        esac
    done < "$list_file"
    if [ "$count" -eq 0 ]; then
        error "Release package is empty."
        return 1
    fi
    if ! tar -tvzf "$archive" > "$verbose_file"; then
        error "Release package metadata could not be inspected."
        return 1
    fi
    if awk '{ print substr($1, 1, 1) }' "$verbose_file" | grep -Ev '^[-d]$' >/dev/null; then
        error "Release package contains a link or special file."
        return 1
    fi
    ...
    ' "$verbose_file"; then
        error "Release package exceeds its expanded byte limit."
        return 1
    fi
}
...
web'
    ...
    if [ "$actual" != "$expected" ] ||
       ...
       [ ! -f "$bundle/VERSION" ] ||
       [ "$(awk 'END { print NR }' "$bundle/VERSION")" -ne 1 ] ||
       [ "$(sed -n '1p' "$bundle/VERSION")" != "$version" ] ||
       [ ! -x "$bundle/install.sh" ] ||
       [ ! -x "$bundle/bin/rstorrent-crostini" ] ||
       [ ! -x "$bundle/bin/rstorrent-gateway" ] ||
       [ ! -f "$bundle/web/index.html" ] ||
       [ ! -f "$bundle/icons/rstorrent-128.png" ]; then
        error "Release package has an invalid bundle shape."
        return 1
    fi
}
...
validate_extracted_bundle() {
    local bundle="$1"
    local version="$2"
    validate_extracted_bundle_shape "$bundle" "$version" || return 1
    if [ "$("$bundle/bin/rstorrent-crostini" --version)" != "rstorrent-crostini ${version}" ]; then
        error "Release package version does not match its signed manifest."
        return 1
    fi
}
...
installer_main() (
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            --version)
                [ "$#" -ge 2 ] || { error "--version requires a value"; return 1; }
                requested_version="${2#v}"
                shift 2
                ;;
            --help|-h)
                printf '%s\n' "Usage: install-crostini.sh [--version X.Y.Z]"
                return 0
                ;;
    ...
    if [ -x "$installed_binary" ]; then
        ...
        if version_is_older "$RELEASE_VERSION" "$installed_version"; then
            error "Refusing to replace installed ${installed_version} with older signed release ${RELEASE_VERSION}."
            return 1
        fi
    fi
    ...
    local listing="$temp_dir/archive.list"
    local bundle="$temp_dir/bundle"
    validate_archive_listing "$asset" "$listing" || return 1
    mkdir "$bundle" || return 1
    tar -xzf "$asset" -C "$bundle" || return 1
    ...
    printf '%s\n' "  rstorrent-crostini uninstall --purge"
)

if [ "${RSTORRENT_CROSTINI_INSTALLER_LIB_ONLY:-}" = "1" ]; then
